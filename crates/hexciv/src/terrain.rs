    ...
    for x in 0..map_size.x {
        for y in 0..map_size.y {
            ...
            .contains(&tile_texture.0)
            {
                ...
                let river_edges = if let Some(_tile_pos_a) = tile_pos_a {
                        ...
                        neighbor_positions.get(HEX_DIRECTIONS[edge_a_mirrored])
                    {
                        ...
                        rivers.iter_mut().find(|river_edges| {
                            river_edges.iter().any(
                                |&RiverEdge {
                                     source,
                                     destination_vertex_direction,
                ...
                let river_edges = if river_edges.is_some() {
                    river_edges
                } else if let Some(_tile_pos_b) = tile_pos_b {
                ...
                // TODO: Check for existing river edges to prepend to.

                // Insert the new river edge.
                let river_edge = RiverEdge {
                    source: tile_pos,
                    destination_vertex_direction: vertex_direction,
                    stream_order: StreamOrder(1),
                };
                if let Some(river_edges) = river_edges {
                    river_edges.push(river_edge);
                } else {
                    rivers.push(vec![river_edge]);
                }
